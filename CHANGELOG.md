...
- Upgrade the pinned runtime to `@deepseek-ai/dsh@0.1.6-alpha.2`. Adopt official Messages defaults, durable Inbox projections, unarchive, Plugin Manager operations, model capabilities, file upload receipts, delivered files and per-turn change snapshots/diffs. Preserve old credentials, model declarations, V2/V3 history and legacy `code` sessions. Remove the obsolete projection-cache and tool-replay patches; retain only the saved-credential endpoint probe adapter.
