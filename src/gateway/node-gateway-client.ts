...
export class NodeGatewayClient {
  ...
  async ensureLegacyCodePreset(): Promise<void> {
    ...
    if (!roster.authorable || roster.presets.some((preset) => preset.id === 'code')
      || !roster.presets.some((preset) => preset.id === 'ptc' && !preset.broken)) return
    await this.callRaw('agentPresets/copy', { from: 'ptc', id: 'code', name: 'PTC (legacy code sessions)' })
  }
